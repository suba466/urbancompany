...
const basePrice = 2195;
 const handleAddToCart = async (pkg, selectedServices = []) => {
  try {
    // Only extra services price
    const extraPrice = selectedServices.reduce((sum, s) => sum + (Number(s.price) || 0), 0);
    ...
    const payload = {
      ...
      content: [
        ...baseServices.map(s => ({
          ...
          details: s.content === "Threading" ? s.title : `${s.title} (${s.content})`,
          price: s.price,
        })),
      ...
      savedSelections: selectedServices, // store only extras
    };
              ...
              <div key={pkg._id || pkg.title}>
                <Row className="align-items-center mt-3">
                  <Col xs={8}>
                    ...
                    <h6 className="fw-semibold" style={{ cursor: "pointer" }}onClick={() => handleShowModal(pkg)}>{pkg.title}</h6>
                  ...
                  <Col xs={4} className="d-flex justify-content-end align-items-center">
                    {!inCart ? (
                      <Button
                       ref={(el) => (addButtonRefs.current[pkg.title.toLowerCase().trim()] = el)}
