...
function Salon1modal({ show, onHide, selectedItem,addButtonRefs,handleAddToCart,basePrice,baseServices}) {
...
useEffect(() => {
  if (show && selectedItem) {
    // Map base services into the same key format
    const baseSelections = {};
    baseServices.forEach(s => {
      const section = s.content === "Threading" ? "facialHairOptions" : "waxingOptions";
      baseSelections[`${section}:${s.title}`] = { ...s, count: 1 };
    });

    // Convert savedSelections array to key-value object
    const savedSelectionsObj = {};
    (selectedItem.savedSelections || []).forEach(s => {
      const section = s.content === "Threading" ? "facialHairOptions" : "waxingOptions";
      savedSelectionsObj[`${section}:${s.title}`] = { ...s, count: 1 };
    });

    const mergedSelections = { ...baseSelections, ...savedSelectionsObj };

    setSelectedServices(mergedSelections);

    const extraPrice = Object.values(savedSelectionsObj).reduce(
      (sum, s) => sum + (s.price || 0),
      0
    );
            ...
            <Row>
              ...
              <Col className="p-3 ">
               <Button className="butn"
                onClick={async () => {
                  const key = selectedItem.title.toLowerCase().trim();
                  const refBtn = addButtonRefs.current[key];
                  console.log(" Checking ref for:", key);
                  console.log("All refs:", addButtonRefs.current);

                  if (refBtn) {
                    console.log("Ref found → clicking Add button");
                    refBtn.click();
                    refBtn.disabled = true;
                    setTimeout(() => (refBtn.disabled = false), 800);
                  } else {
                    console.warn("Ref not found, adding directly...");
                    if (typeof handleAddToCart === "function") {
                      await handleAddToCart(selectedItem,Object.values(selectedServices));
                    }
                  }
